...
@Component({
  ...
  template: `
    <div class="bg-blue-50 p-8 rounded-lg mb-8">
      <h1 class="text-4xl font-bold mb-4">Welcome to Convivio</h1>
      <p class="text-lg mb-6">Discover events, places, and connect with people who share your interests.</p>
      <div class="flex gap-4">
        <a routerLink="/events" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">Browse Events</a>
        <a routerLink="/places" class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700">Explore Places</a>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <section>
        <div class="flex justify-between items-center mb-4">
          ...
          <a routerLink="/events" class="text-blue-600 hover:underline">View all</a>
        </div>
          ...
          @if (upcomingEvents.length > 0) {
            @for (event of upcomingEvents; track event.id) {
              <div class="bg-white p-4 rounded-lg shadow-md">
                ...
                <a [routerLink]="['/events', event.id]" class="text-blue-600 hover:underline mt-2 inline-block">View details</a>
              </div>
            }
          } @else {
      ...
      <section>
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-2xl font-bold">Popular Places</h2>
          <a routerLink="/places" class="text-blue-600 hover:underline">View all</a>
        </div>
